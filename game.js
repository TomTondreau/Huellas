...
function animate() {
    ...
    // Lógica de generación procedural de árboles
    const cameraZ = camera.position.z;
    const generationDistance = 100; // Distancia a la que generamos nuevos árboles y rocas
    const despawnDistance = 50; // Distancia a la que los objetos desaparecen detrás de la cámara

    // Eliminar árboles que están muy atrás y generar nuevos
    for (let i = activeTrees.length - 1; i >= 0; i--) {
        const tree = activeTrees[i];
        // Si el árbol está muy por detrás de la cámara, lo movemos a la piscina
        if (tree.position.z > cameraZ + despawnDistance) {
            trees.remove(tree);
            activeTrees.splice(i, 1);
            treePool.push(tree);
...
// --- Sistema de Partículas (Hojas Cayendo) ---
const leafCount = 500; // Número de hojas
const leavesGeometry = new THREE.BufferGeometry();
const positions = [];
const colors = [];

const leafColor = new THREE.Color(0xcc8800); // Color de hoja otoñal

for (let i = 0; i < leafCount; i++) {
    // Posiciones aleatorias iniciales
    positions.push(
        (Math.random() - 0.5) * 100, // X
        Math.random() * 50,         // Y (altura)
        (Math.random() - 0.5) * 200  // Z
    );
    colors.push(leafColor.r, leafColor.g, leafColor.b);
}

leavesGeometry.setAttribute('position', new THREE.Float32BufferAttribute(positions, 3));
leavesGeometry.setAttribute('color', new THREE.Float32BufferAttribute(colors, 3));

const leavesMaterial = new THREE.PointsMaterial({
    size: 0.5, // Tamaño de cada hoja
    vertexColors: true, // Usar los colores definidos
    blending: THREE.AdditiveBlending, // Para que se vean bien sobre el fondo
    transparent: true,
    opacity: 0.8
});

const leafParticles = new THREE.Points(leavesGeometry, leavesMaterial);
